...
```{r, echo=FALSE}
p1 <- old |> ggplot(aes(x = acousticness, y = energy, text = track.name)) + 
  geom_point() + geom_smooth() + labs(title = "U2 before the 90's")
p1 <- ggplotly(p1, tooltip = c("x", "y", "text"))
p1 <- layout(p1, title = "U2 before the 90's")
p1 <- p1 %>% layout(title = "U2 before the 90's")
```

```{r, echo=FALSE}
p2 <- U290s |> ggplot(aes(x = acousticness, y = energy, text = track.name)) + 
  geom_point() + geom_smooth() + labs(title = "U2 in the 90's")
p2 <- ggplotly(p2, tooltip = c("x", "y", "text"))
p2 <- layout(p2, title = "U2 in the 90's")

subplot(p1, p2, nrows = 1)
```


### Chroma features for "The First Time"
...
On the visualisation page, the graph on the left shows the relation between energy and acousticness in the songs by U2 from before 1990 and the graph on the right does the same for the 90's period.This showcases that the experimental phase of U2 in the 90's goes hand in hand with a decrease in overall acousticness, whilst maintaining the energy present in their earlier work. The outlier on the bottom right is a song called 'October', 
